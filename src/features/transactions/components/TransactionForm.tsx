...
export function TransactionForm({ onSubmit, creating }: TransactionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(form);
      setSuccess(true);
      setForm({ tipo: 'GASTO', monto: 0, fecha: new Date().toISOString().split('T')[0], categoriaId: 5 });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="monto" className="text-slate-300 text-sm">Monto (COP)</Label>
        <Input
          id="monto"
          type="number"
          min="1"
      ...
      <div className="space-y-1.5">
        <Label htmlFor="fecha" className="text-slate-300 text-sm">Fecha</Label>
        <Input
          id="fecha"
          type="date"
          value={form.fecha}
      ...
      <div className="space-y-1.5">
        <Label htmlFor="categoria" className="text-slate-300 text-sm">Categoría</Label>
        <select
          id="categoria"
          value={form.categoriaId}
          onChange={(e) => update('categoriaId', parseInt(e.target.value))}
